...
.sb-hero-left .lbl{margin:0 0 3px;font-size:8px;letter-spacing:1.8px;font-weight:700;text-transform:uppercase;color:var(--ind-color,#0EA5E9)}
...
.sb-hero-left .persona-role{margin:2px 0 0;font-size:10.5px;color:#94A3B8;line-height:1.45}
.sb-hero-left .mission-text{margin:0;font-size:10.5px;color:#CBD5E1;line-height:1.6}
.sb-hero-left .punch{margin-top:auto;padding:10px;background:rgba(255,255,255,0.06);border-radius:8px;border:1px solid rgba(255,255,255,0.12)}
...
.sb-phase{margin-bottom:18px;padding:14px 14px 12px;background:#F8FAFC;border:1px solid var(--border);border-radius:8px;border-left:3px solid var(--ind-color,var(--blue))}
.sb-phase:last-child{margin-bottom:0}
.sb-phase-head{display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap}
.sb-phase-title{font-size:14px;font-weight:700;color:var(--text);line-height:1.3}
.sb-phase-summary{margin:0;font-size:12.5px;color:#475569;line-height:1.55}
...
function _translateLocale(s, src){
  ...
  if(_locale==='MY_BM' && src==='EN'){
    for(let i=0;i<_TRANSLATE_EN_TO_BM.length;i++){ out = out.replace(_TRANSLATE_EN_TO_BM[i][0], _TRANSLATE_EN_TO_BM[i][1]); }
  } else if(_locale==='ID_EN' && src==='BI'){
    for(let i=0;i<_TRANSLATE_BI_TO_EN.length;i++){ out = out.replace(_TRANSLATE_BI_TO_EN[i][0], _TRANSLATE_BI_TO_EN[i][1]); }
  }
  // Tidy spacing
  out = out.replace(/[ \t]{2,}/g,' ').replace(/\s+([,.;:])/g,'$1');
  return out;
...
const OTHER_PAGES = [
  ...
  },
  {
    ...
    password: 'ABMB7May',
    tag: 'Bank Workshop',
    keywords: 'ABMB Alliance Bank Malaysia prompt-a-thon banking SME HR finance risk digital comms strategy'
  },
  {
