...
.sidebar-search-input{width:100%;padding:8px 30px 8px 32px;font-size:12px;border:1px solid var(--border);border-radius:8px;background:#FFFFFF;color:var(--text);font-family:inherit;outline:none;transition:all 0.18s}
...
.sidebar-search-clear{position:absolute;right:22px;top:50%;transform:translateY(-50%);background:#E2E8F0;color:#475569;border:none;border-radius:50%;width:18px;height:18px;font-size:11px;line-height:1;cursor:pointer;display:none;align-items:center;justify-content:center;font-family:inherit;padding:0}
.sidebar-search-clear.show{display:flex}
.sidebar-search-clear:hover{background:#CBD5E1;color:var(--navy)}
.sidebar-search-hint{position:absolute;right:30px;top:50%;transform:translateY(-50%);font-size:9px;color:#94A3B8;background:#F1F5F9;border:1px solid var(--border);border-radius:4px;padding:1px 5px;font-weight:600;pointer-events:none}
...
.op-card-team-group{display:flex;flex-direction:column;gap:4px}
.op-card-team-group-hdr{font-size:9.5px;font-weight:800;color:var(--tg-color,#0EA5E9);text-transform:uppercase;letter-spacing:0.6px;padding-bottom:3px;border-bottom:1px dashed rgba(14,165,233,0.25);margin-bottom:2px}
.op-card-team-row{display:grid;grid-template-columns:1fr auto auto;align-items:center;gap:6px;font-size:10px;line-height:1.35;padding:1px 0}
.op-card-team-name{color:#1E293B;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.op-card-team-code{font-family:'Courier New',monospace;font-size:9.5px;color:#9A3412;background:#FEF3C7;border:1px solid #FDE68A;border-radius:6px;padding:1px 7px;font-weight:700;cursor:pointer;flex-shrink:0;transition:transform 0.12s,box-shadow 0.12s;white-space:nowrap}
.op-card-team-code:hover{transform:translateY(-1px);box-shadow:0 2px 5px rgba(154,52,18,0.18)}
.op-card-team-link{font-size:11px;color:var(--op-color,#0078D4);text-decoration:none;font-weight:800;padding:1px 4px;border-radius:4px;transition:background 0.12s}
...
.sidebar-item:hover{background:#F8FAFC;color:var(--blue);text-decoration:none}
...
.sb-hero{display:grid;grid-template-columns:240px 1fr;gap:0;border:1px solid var(--border);border-radius:14px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.06);background:#FFFFFF}
...
.sb-hero-right{background:#FFFFFF;padding:18px 16px 14px;min-width:0}
...
const _UI = {
  ...
  // Home subtitle
  'Select an industry or department to explore M365 Copilot demo scenarios, prompts, and reference files':
                           ['Pilih industri atau departemen untuk mengeksplorasi skenario demo M365 Copilot, prompt, dan file referensi',
                            'Pilih industri atau jabatan untuk meneroka senario demo M365 Copilot, gesaan dan fail rujukan'],
  // Detail page chrome
  ...
  '🛠️ M365 Copilot Tools':  ['🛠️ Tools M365 Copilot',                  '🛠️ Alat M365 Copilot'],
  ...
  '✨ M365 Copilot — chat-based continuous-motion flow (Word Agent → PPT Agent → Excel Agent → Agent Builder)':
                           ['✨ M365 Copilot — alur kerja berbasis chat berkelanjutan (Word Agent → PPT Agent → Excel Agent → Agent Builder)',
                            '✨ M365 Copilot — aliran kerja berasaskan chat berterusan (Word Agent → PPT Agent → Excel Agent → Agent Builder)'],
  ...
  '✨ M365 Copilot':       ['✨ M365 Copilot',                        '✨ M365 Copilot'],
...
function _applyLocaleLabels(){
  ...
  if(homeSub) homeSub.textContent=_uL('Select an industry or department to explore M365 Copilot demo scenarios, prompts, and reference files');
}
...
function _translateLocale(s, src){
  if(!s) return s;
  src = src || 'EN';
  // Native baked translations: skip phrase substitution entirely.
  if(src === 'BM_NATIVE' || src === 'EN_NATIVE') return String(s);
  let out = String(s);
  if(_locale==='MY_BM' && src==='EN'){
    out = _applyDict(out, _TRANSLATE_EN_TO_BM);
  } else if(_locale==='ID_EN' && src==='BI'){
    out = _applyDict(out, _TRANSLATE_BI_TO_EN);
  }
  // Tidy spacing
  out = out.replace(/[ \t]{2,}/g,' ').replace(/\s+([,.;:])/g,'$1');
  return out;
...
//   MY_BM with native *BM field → returns *BM, src='BM_NATIVE' (translator skips)
//   ID_EN with native *IDEN field → returns *IDEN, src='EN_NATIVE' (translator skips)
//   ID_BI / ID_EN with *ID field → returns *ID, src='BI'
//   else → returns base field, src='EN'
function _pickField(item, base){
  if(_locale==='MY_BM'){
    const bmVal = item[base+'BM'];
    if(bmVal) return {val:bmVal, src:'BM_NATIVE'};
  }
  if(_locale==='ID_EN'){
    const idenVal = item[base+'IDEN'];
    if(idenVal) return {val:idenVal, src:'EN_NATIVE'};
  }
  const idVal = item[base+'ID'];
  if((_locale==='ID_BI' || _locale==='ID_EN') && idVal) return {val:idVal, src:'BI'};
...
function _getPrompts(t){
  ...
  else if(_locale==='ID_EN' && t.promptsID && t.promptsID.length){ arr=t.promptsID; src='BI'; }
...
function _getPersona(t,pi){
  ...
  else if((_locale==='ID_BI' || _locale==='ID_EN') && t.personaID && t.personaID.length){ arr=t.personaID; src='BI'; }
  else arr=(t.persona||[]);
...
function showItem(item,tab){
  ...
  if(hasChat && hasOther){
    ...
    tabsWrap.innerHTML=
      '<button class="detail-tab '+( _detailTab==='tools'?'active':'')+'" id="dtab-tools" onclick="setDetailTab(\'tools\')">'+_uL('🛠️ M365 Copilot Tools')+'</button>'+
  ...
  visibleTools.forEach((tool)=>{
    ...
    if(_detailTab==='chat'){
      ...
      if(grp!==_lastGroup){
        ...
        hdr.innerHTML=grp==='free'
          ...
          :_uL('✨ M365 Copilot — chat-based continuous-motion flow (Word Agent → PPT Agent → Excel Agent → Agent Builder)');
        pEl.appendChild(hdr);
        _lastGroup=grp;
      }
    }
    ...
    let licClass='premium', licLabel=_uL('✨ M365 Copilot');
...
const OTHER_PAGES = [
  ...
  },
  {
    ...
    desc: 'HR-focused Copilot immersion for a Malaysian conglomerate (5 May 2026). Talent, recruitment, performance, payroll workflows with M365 Copilot.',
  ...
  },
  {
    ...
    desc: 'Trade & cross-border HR scenarios using paid M365 Copilot features (Researcher, Analyst, Frontier Cowork, Agent Builder).',
...
function buildOtherGrid(){
  ...
  intro.innerHTML='<div class="detail-scenario-title">&#128218; Other Copilot Demo Pages</div>'+
    '<div class="detail-scenario-text">A library of related M365 Copilot demo sites we have built for other customers and workshops. Each link opens in a new tab — click the &#128274; password chip on a card to copy the password to your clipboard.</div>';
