...
const _SCRUB_REAL = [
  ...
  [/\bBank Negara Malaysia\b/gi, 'the central bank'],
  [/\bBNM\b/g, 'the central bank'],
  [/\bBank Indonesia\b/gi, 'the central bank'],
  [/\bSecurities Commission Malaysia\b/gi, 'the securities regulator'],
  [/\bSC Malaysia\b/gi, 'the securities regulator'],
  [/\bSuruhanjaya Sekuriti\b/gi, 'the securities regulator'],
  [/\bBursa Malaysia\b/gi, 'the local stock exchange'],
  [/\bBursa\b/g, 'the local stock exchange'],
  [/\bOJK\b/g, 'the financial services authority'],
  [/\bOtoritas Jasa Keuangan\b/gi, 'the financial services authority'],
  [/\bIDX\b/g, 'the local stock exchange'],
  [/\bBEI\b/g, 'the local stock exchange'],
  [/\bMatrade\b/gi, 'the trade authority'],
  [/\bMIDA\b/g, 'the investment authority'],
  [/\bBKPM\b/g, 'the investment authority'],
  [/\bMPOB\b/g, 'the palm-oil authority'],
  [/\bSPAN\b/g, 'the water-services regulator'],
  [/\bMCMC\b/g, 'the communications regulator'],
  [/\bKominfo\b/g, 'the communications ministry'],
  [/\bLHDN\b/g, 'the tax authority'],
  [/\bDJP\b/g, 'the tax authority'],
];

// Global-only swaps (only fired when _isGlobal()). Strip MY/ID geographic + currency markers.
const _SCRUB_GLOBAL_GEO = [
  [/\bMalaysia(n)?\b/gi, 'the region'],
  [/\bIndonesia(n)?\b/gi, 'the region'],
  [/\bASEAN\b/g, 'the region'],
  [/\bSEA\b/g, 'the region'],
  [/\bSouth-?east\s+Asia\b/gi, 'the region'],
  [/\bMYR\s*([\d.,]+)/gi, 'USD $1'],
  [/\bRM\s*([\d.,]+)/gi, '$$$1'],
  [/\bIDR\s*([\d.,]+)/gi, 'USD $1'],
  [/\bRp\s*([\d.,]+)/gi, '$$$1'],
  ...
  [/\bJohor\b/g, 'a coastal state'],
  [/\bPenang\b/g, 'a coastal state'],
  [/\bSelangor\b/g, 'the central state'],
  [/\bSabah\b/g, 'a remote state'],
  [/\bSarawak\b/g, 'a remote state'],
  [/\bSurabaya\b/gi, 'a coastal city'],
  [/\bBandung\b/gi, 'an inland city'],
  [/\bMedan\b/gi, 'a coastal city'],
...
//   Country selects MY/ID base field. Language selects prompt array.
//   In Global mode, picks MY base field then runs full scrub (incl. geo).
function _pickLangField(item, base){
  const idVal = item[base+'ID'];
  if(_isID() && idVal) return idVal;
  return item[base]; // MY default & GLOBAL default
}
function _getCompany(item){return _scrubReal(_pickLangField(item,'company'));}
function _getTagline(item){return _scrubReal(_pickLangField(item,'tagline'));}
function _getScenario(item){return _scrubReal(_pickLangField(item,'scenario'));}
function _getPrompts(t){
  let arr;
  if(_langCode()==='BM' && t.promptsBM && t.promptsBM.length) arr = t.promptsBM;
  else if(_langCode()==='BI' && t.promptsID && t.promptsID.length) arr = t.promptsID;
  else arr = t.prompts || [];
  return arr.map(p=>{
    if(typeof p === 'string') return _scrubReal(p);
    const r = Object.assign({}, p);
    if(r.instr) r.instr = _scrubReal(r.instr);
    if(r.prompt) r.prompt = _scrubReal(r.prompt);
    return r;
  });
}
function _getPersona(t,pi){
  let arr=null;
  if(_langCode()==='BM' && t.personaBM && t.personaBM.length) arr=t.personaBM;
  else if(_isBahasa() && t.personaID && t.personaID.length) arr=t.personaID;
  else arr=(t.persona||[]);
  return _scrubReal(arr[pi]||'');
}
_syncLegacyLang();
...
function showItem(item,tab){
  ...
  if(item.storyboard && item.storyboard.length){
    ...
    const personaRole=p0?(_isID()&&p0.roleID?p0.roleID:p0.role):'';
    const personaCompany=p0?_scrubReal(p0.company||''):'';
    // Mission text: prefer tagline, fall back to first sentence of scenario
    let missionText='';
    if(_isID()&&item.taglineID) missionText=item.taglineID;
    else if(item.tagline) missionText=item.tagline;
    else if(_isID()&&item.scenarioID) missionText=item.scenarioID.split(/[.!?]/)[0]+'.';
    else if(item.scenario) missionText=item.scenario.split(/[.!?]/)[0]+'.';
    missionText=_scrubReal(_scrubFiles(missionText));
    ...
    const phasesHTML=item.storyboard.map((ex,i)=>{
      const exTitle=_isID()&&ex.titleID?ex.titleID:(ex.title||'');
      const exSummary=_isID()&&ex.summaryID?ex.summaryID:(ex.summary||'');
      const cleanSummary=_scrubReal(_scrubFiles(exSummary));
      return '<div class="sb-phase">'+
        '<div class="sb-phase-head">'+
          '<span class="sb-phase-title">'+escapeHTML(exTitle)+'</span>'+
        '</div>'+
  ...
  visibleTools.forEach((tool)=>{
    ...
    const promHtml=promptArr.map((p,pi)=>{
      ...
      if(isNotebook && pi===0 && tool.notebookMeta){
        ...
        const instr=_isID()&&nm.instructionsID?nm.instructionsID:(nm.instructions||'');
        notebookHtml=
  ...
  if(item.personas && item.personas.length){
    ...
    personasEl.innerHTML=
      ...
      item.personas.map(p=>{
        const role=_isID()&&p.roleID?p.roleID:p.role;
