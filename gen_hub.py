...
//   country=ID locales (ID_BI/ID_EN) → returns *ID variant if present, src='BI'
...
function showItem(item,tab){
  ...
  document.getElementById('detail-badge').textContent=isDept?_uL('Department'):_stripLeadingEmoji(item.name);
  ...
  if(item.storyboard && item.storyboard.length){
    ...
    // Mission text: prefer tagline, fall back to first sentence of scenario
    let missionText='', missionSrc='EN';
    if((_locale==='ID_BI'||_locale==='ID_EN') && item.taglineID){ missionText=item.taglineID; missionSrc='BI'; }
    else if(item.tagline){ missionText=item.tagline; missionSrc='EN'; }
    else if((_locale==='ID_BI'||_locale==='ID_EN') && item.scenarioID){ missionText=item.scenarioID.split(/[.!?]/)[0]+'.'; missionSrc='BI'; }
    else if(item.scenario){ missionText=item.scenario.split(/[.!?]/)[0]+'.'; missionSrc='EN'; }
    ...
    const phasesHTML=item.storyboard.map((ex,i)=>{
      let exTitle='', exSummary='', exSrc='EN';
      if((_locale==='ID_BI'||_locale==='ID_EN') && ex.titleID){ exTitle=ex.titleID; exSrc='BI'; }
      else { exTitle=ex.title||''; }
      let summarySrc='EN';
      if((_locale==='ID_BI'||_locale==='ID_EN') && ex.summaryID){ exSummary=ex.summaryID; summarySrc='BI'; }
...
const OTHER_PAGES = [
  ...
  },
  {
    ...
    desc: 'Alliance Bank Malaysia × Microsoft 365 Copilot Prompt-a-thon (7 May 2026). 8 team packs across Consumer Banking, SME, HR, Finance, Risk, Digital, Comms, and Strategy.',
    url: 'https://steffiyappy.github.io/ABMB7May/',
    password: 'ABMB7May',
    tag: 'Hackathon',
    keywords: 'ABMB Alliance Bank Malaysia prompt-a-thon hackathon banking SME HR finance risk digital comms strategy'
  },
  {
  ...
  },
  {
    ...
    desc: 'Cagamas x Microsoft Copilot Challenge Week — Prompt-a-thon (12 May 2026) and Agent-a-thon (13 May 2026) materials.',
    ...
    tag: 'Hackathon',
    keywords: 'cagamas mortgage finance prompt-a-thon agent-a-thon hackathon may 2026 challenge'
  }
];
    ...
    sections[secName].forEach(p=>{
      ...
      card.innerHTML=
        ...
        '<div class="op-card-body">'+
          ...
          '</div>'+
        '</div>';
      // Wire click-to-copy for every password chip on this card
      card.querySelectorAll('.op-card-pwd[data-pwd]').forEach(pwdEl=>{
        pwdEl.addEventListener('click', function(e){
          e.preventDefault(); e.stopPropagation();
          const val=pwdEl.getAttribute('data-pwd')||'';
          navigator.clipboard.writeText(val).then(()=>{
            const t=document.getElementById('toast');
            t.textContent='\u2713 Password copied: '+val;
