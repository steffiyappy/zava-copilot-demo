...
.op-card-meta{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-top:auto;padding-top:10px;border-top:1px solid var(--border)}
.op-card-pwd{font-size:10px;color:#9A3412;background:#FEF3C7;border:1px solid #FDE68A;border-radius:10px;padding:2px 8px;font-weight:600;font-family:'Courier New',monospace}
.op-card-pwd.none{background:#F0FDF4;border-color:#BBF7D0;color:#166534}
.op-card-link{display:inline-flex;align-items:center;gap:4px;font-size:11px;font-weight:700;color:var(--op-color,var(--blue));text-decoration:none;transition:gap 0.15s}
...
.ind-card,.dept-pill,.sb-task,.sb-phase,.tool-header,.tool-prompts,.copy-btn{transition:all 0.18s cubic-bezier(.2,.8,.2,1)}
...
.sb-phase{margin-bottom:14px}
.sb-phase:last-child{margin-bottom:0}
.sb-phase-head{display:flex;align-items:center;gap:8px;margin-bottom:7px;flex-wrap:wrap}
.sb-phase-num{background:#0F2545;color:#FFFFFF;font-size:9px;font-weight:700;padding:3px 9px;border-radius:4px;letter-spacing:0.5px}
.sb-phase-title{font-size:12px;font-weight:700;color:var(--text)}
.sb-phase-mins{font-size:10px;color:var(--muted);font-weight:600}
.sb-task-grid{display:grid;gap:8px}
.sb-task-grid.cols-1{grid-template-columns:1fr}
.sb-task-grid.cols-2{grid-template-columns:repeat(2,1fr)}
.sb-task-grid.cols-3{grid-template-columns:repeat(3,1fr)}
@media(max-width:760px){.sb-hero{grid-template-columns:1fr}.sb-task-grid.cols-2,.sb-task-grid.cols-3{grid-template-columns:1fr}}
.sb-task{border:1px solid var(--border);border-radius:7px;overflow:hidden;background:#FFFFFF;cursor:pointer;display:flex;flex-direction:column;min-width:0}
.sb-task:hover{transform:translateY(-1px);box-shadow:0 6px 16px rgba(0,0,0,0.08);border-color:var(--ind-color,var(--blue))}
.sb-task-head{padding:5px 9px;display:flex;align-items:center;gap:5px;color:#FFFFFF}
.sb-task-num{font-size:9.5px;font-weight:800;color:rgba(255,255,255,0.55);min-width:14px}
.sb-task-tool{font-size:8.5px;font-weight:700;text-transform:uppercase;letter-spacing:0.3px;color:#FFFFFF;line-height:1.2;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.sb-task-body{padding:8px 10px 10px;background:#F8FAFC;flex:1;display:flex;flex-direction:column;gap:2px}
.sb-task-title{margin:0;font-size:11px;font-weight:700;color:var(--text);line-height:1.3}
.sb-task-label{margin:0 0 4px;font-size:9px;font-style:italic;color:var(--ind-color,var(--blue));font-weight:600}
.sb-task-narr{margin:0;font-size:9.5px;color:var(--muted);line-height:1.5}
...
function applySearch(q){
  ...
  document.querySelectorAll('#sidebar-industries .sidebar-item').forEach(el=>{
    ...
    let match=!query;
    if(query && ind){
      const hay=(name+' '+(ind.company||'')+' '+(ind.companyID||'')+' '+(ind.tagline||'')+' '+(ind.taglineID||'')).toLowerCase();
  ...
  document.querySelectorAll('#sidebar-departments .sidebar-item').forEach(el=>{
    ...
    let match=!query;
    if(query && dept){
      const hay=(name+' '+(dept.company||'')+' '+(dept.tagline||'')).toLowerCase();
      match=hay.indexOf(query)>=0;
    } else if(query){
      match=name.indexOf(query)>=0;
    }
  ...
  document.querySelectorAll('#ind-grid .ind-card').forEach(card=>{
    ...
    card.style.display=(!query || txt.indexOf(query)>=0)?'block':'none';
  });
...
function showItem(item,tab){
  ...
  if(item.storyboard && item.storyboard.length){
    ...
    const personaName=p0?p0.name:'';
    const personaRole=p0?(_lang==='ID'&&p0.roleID?p0.roleID:p0.role):'';
    const personaCompany=p0?(p0.company||''):'';
    // Mission text: prefer tagline, fall back to first sentence of scenario
    let missionText='';
    if(_lang==='ID'&&item.taglineID) missionText=item.taglineID;
    else if(item.tagline) missionText=item.tagline;
    else if(_lang==='ID'&&item.scenarioID) missionText=item.scenarioID.split(/[.!?]/)[0]+'.';
    else if(item.scenario) missionText=item.scenario.split(/[.!?]/)[0]+'.';
    missionText=_scrubFiles(missionText);
    // Total tools used = count unique across all phases (for Punchline line)
    const allTools=new Set();
    item.storyboard.forEach(ex=>(ex.tasks||[]).forEach(t=>allTools.add(_toolLabelShort(t))));
    const toolsCount=allTools.size||(item.prompts?item.prompts.length:8);
    // Total tasks (for right-header)
    let totalTasks=0;
    item.storyboard.forEach(ex=>{totalTasks+=(ex.tasks||[]).length;});
    const labelCopilot=_lang==='ID'?'Copilot di M365':'Copilot in M365';
    const labelHeroTitle=_lang==='ID'?'Asisten AI<br>menjadi Pelaku AI':'AI Assistant<br>to AI Doer';
    const labelPersona=_lang==='ID'?'Persona':'Persona';
    const labelMission=_lang==='ID'?'Misi Hari Ini':'The Mission';
    const labelPunch=_lang==='ID'?'Inti Cerita':'The Punchline';
    const punchText=_lang==='ID'
      ?('Satu hari kerja. '+toolsCount+' alat. Dari konteks ke pengiriman.')
      :('One workday. '+toolsCount+' tools. From frame to ship.');
    const rightHeader=_lang==='ID'
      ?(totalTasks+' babak dalam satu hari kerja')
      :(totalTasks+' moments across one workday');
    ...
    const phasesHTML=item.storyboard.map((ex,i)=>{
      ...
      const colsCls=_gridColsCls(tasks.length);
      const tasksHTML=tasks.map((t,ti)=>{
        const verb=_lang==='ID'&&t.verbID?t.verbID:(t.verb||'');
        const toolColor=_toolColor(t);
        const toolLabel=_toolLabelShort(t);
        const stepNum=String(ti+1).padStart(2,'0');
        const stepLabel=(_lang==='ID'?'Tugas ':'Task ')+(i+1)+'.'+(ti+1);
        const split=_verbSplit(verb);
        return '<div class="sb-task" data-tool="'+escapeAttr(t.tool||t.toolId||'')+'">'+
          '<div class="sb-task-head" style="background:'+toolColor+'">'+
            '<span class="sb-task-num">'+stepNum+'</span>'+
            '<span class="sb-task-tool">'+escapeHTML(toolLabel)+'</span>'+
          '</div>'+
          '<div class="sb-task-body">'+
            '<p class="sb-task-title">'+escapeHTML(split.title)+'</p>'+
            '<p class="sb-task-label">'+escapeHTML(stepLabel)+'</p>'+
            (split.body?'<p class="sb-task-narr">'+escapeHTML(split.body)+'</p>':'')+
          '</div>'+
          '</div>';
      }).join('');
      const minsLabel=_lang==='ID'?' menit':' min';
      return '<div class="sb-phase">'+
        '<div class="sb-phase-head">'+
          '<span class="sb-phase-num">'+(_lang==='ID'?'BABAK ':'EXERCISE ')+(i+1)+'</span>'+
          (ex.minutes?'<span class="sb-phase-mins">&middot; '+ex.minutes+minsLabel+'</span>':'')+
          '<span class="sb-phase-title">&middot; '+escapeHTML(exTitle)+'</span>'+
        '</div>'+
        '<div class="sb-task-grid '+colsCls+'">'+tasksHTML+'</div>'+
        '</div>';
    }).join('');
    sbWrap.innerHTML=
...
const OTHER_PAGES = [
  ...
  },
  {
    ...
    desc: 'HR-focused Copilot immersion for a Malaysian conglomerate. Talent, recruitment, performance, payroll workflows with M365 Copilot.',
  ...
  },
  {
    ...
    desc: 'Finance, procurement, and capital-allocation Copilot scenarios for a medical-devices customer. Variance analysis, supplier negotiation, board pack.',
    url: 'https://steffiyappy.github.io/contoso-medical-finance-immersion/',
    password: '',
    tag: 'Finance Workshop',
    keywords: 'contoso medical finance procurement variance supplier capital board pack'
  },
  {
...
function buildOtherGrid(){
  ...
  intro.innerHTML='<div class="detail-scenario-title">&#128218; Other Copilot Demo Pages</div>'+
    '<div class="detail-scenario-text">A library of related M365 Copilot demo sites we have built for other customers and workshops. Each link opens in a new tab — sites may have their own password.</div>';
  ...
  Object.keys(sections).forEach(secName=>{
    ...
    sections[secName].forEach(p=>{
      ...
      const pwdHTML = p.password
        ? '<span class="op-card-pwd" title="Click to copy">&#128274; '+p.password+'</span>'
        : '<span class="op-card-pwd none">&#10003; Open access</span>';
      card.innerHTML=
        '<div class="op-card-banner">'+p.icon+'</div>'+
        '<div class="op-card-body">'+
          '<div class="op-card-tag">'+p.tag+'</div>'+
          '<div class="op-card-name">'+p.name+'</div>'+
          '<div class="op-card-desc">'+p.desc+'</div>'+
          '<div class="op-card-meta">'+pwdHTML+
            '<span class="op-card-link">Open &#8599;</span>'+
          '</div>'+
        '</div>';
      // Click password chip → copy to clipboard
      const pwdEl=card.querySelector('.op-card-pwd');
      if(p.password && pwdEl){
        pwdEl.addEventListener('click', function(e){
          e.preventDefault(); e.stopPropagation();
          navigator.clipboard.writeText(p.password).then(()=>{
            const t=document.getElementById('toast');
            t.textContent='\u2713 Password copied: '+p.password;
