...
let _lang='EN'; // 'EN' | 'ID' (Bahasa Indonesia) | 'BM' (Bahasa Malaysia). Promp data has promptsID + promptsBM; other fields BM falls back to ID.
...
function setCtx(c){ setLang(c==='ID'?'ID':(c==='BM'?'BM':'EN')); }

// Treat ID and BM as Bahasa-family for UI label strings that have no BM-specific copy.
function _isBahasa(){return _lang==='ID' || _lang==='BM';}

// 3-way label picker. If `bm` omitted, BM falls back to BI.
function _bL(en, id, bm){
  if(_lang==='BM') return (bm!==undefined?bm:id);
  if(_lang==='ID') return id;
  return en;
}

function _pickLangField(item, base){
  // BM → use BM field if exists, else fall back to ID, else EN
  if(_lang==='BM'){
    const bm=item[base+'BM']; if(bm) return bm;
    const id=item[base+'ID']; if(id) return id;
    return item[base];
  }
  if(_lang==='ID'){
    const id=item[base+'ID']; if(id) return id;
...
function _getPrompts(t){
  ...
  if((_lang==='ID' || _lang==='BM') && t.promptsID && t.promptsID.length) return t.promptsID;
  return t.prompts||[];
}
function _getPersona(t,pi){
  let arr=null;
  if(_lang==='BM' && t.personaBM && t.personaBM.length) arr=t.personaBM;
  else if((_lang==='ID' || _lang==='BM') && t.personaID && t.personaID.length) arr=t.personaID;
  else arr=(t.persona||[]);
  return arr[pi]||'';
}
...
function showItem(item,tab){
  ...
  if(item.storyboard && item.storyboard.length){
    ...
    const personaRole=p0?(_isBahasa()&&p0.roleID?p0.roleID:p0.role):'';
    const personaCompany=p0?(p0.company||''):'';
    // Mission text: prefer tagline, fall back to first sentence of scenario
    let missionText='';
    if(_isBahasa()&&item.taglineID) missionText=item.taglineID;
    else if(item.tagline) missionText=item.tagline;
    else if(_isBahasa()&&item.scenarioID) missionText=item.scenarioID.split(/[.!?]/)[0]+'.';
    ...
    const phasesHTML=item.storyboard.map((ex,i)=>{
      const exTitle=_isBahasa()&&ex.titleID?ex.titleID:(ex.title||'');
      const exSummary=_isBahasa()&&ex.summaryID?ex.summaryID:(ex.summary||'');
      const cleanSummary=_scrubFiles(exSummary);
  ...
  visibleTools.forEach((tool)=>{
    ...
    const promHtml=promptArr.map((p,pi)=>{
      ...
      if(isNotebook && pi===0 && tool.notebookMeta){
        ...
        const instr=_isBahasa()&&nm.instructionsID?nm.instructionsID:(nm.instructions||'');
        notebookHtml=
  ...
  if(item.personas && item.personas.length){
    ...
    personasEl.innerHTML=
      ...
      item.personas.map(p=>{
        const role=_isBahasa()&&p.roleID?p.roleID:p.role;
