...
let wnIdx=0;
let currentGridTab='ind'; // 'ind' or 'dept'
...
function makeCard(item,tab){
  ...
  div.innerHTML=
    ...
    '<div class="ind-card-company">'+item.company+'</div>'+
...
function showItem(item,tab){
  ...
  document.getElementById('detail-title').textContent=item.company;
  ...
  item.prompts.forEach((tool,ti)=>{
    ...
    const promHtml=tool.prompts.map((p,pi)=>{
      ...
      return '<div class="prompt-item">'+
        ...
        (p.fileRef?'<div class="prompt-fileref">'+escapeHTML(p.fileRef)+'</div>':'')+
