...
# ---------------- INTERNAL AUDIT / RISK CONGLOMERATE POWER MOVES ----------------

CARDS['uc-audit-group-risk-consolidation'] = {
    ...
    'complexity': 'advanced',
    'apps': ['Cowork', 'Excel', 'Analyst', 'Word', 'PowerPoint'],
    ...
    'sample_files': [
        ('Risk_Register_Hospitals.xlsx', 'xlsx'),
        ('Risk_Register_Clinics.xlsx', 'xlsx'),
        ('Risk_Register_Diagnostics.xlsx', 'xlsx'),
        ('Risk_Register_Insurance.xlsx', 'xlsx'),
        ('Incident_Log_FY26.xlsx', 'xlsx'),
    ],
    'prompts': [
        {
            'label': '1. Excel — normalise + consolidate 4 subsidiary registers',
            'text': (
                "Open the 4 subsidiary risk registers in /Subsidiary_Risk_Registers/ (Hospitals, Clinics, Diagnostics, Insurance). Each uses a slightly different taxonomy for likelihood, impact, category, and control state. Produce:\n"
                "(1) A normalised 5x5 likelihood x impact scoring key applied consistently across the 4 registers. Show the mapping used for each subsidiary (source label -> normalised score) so the auditor can trace it.\n"
                "(2) A consolidated Group risk register with columns: Group Risk ID, Subsidiary, Category, Description, Inherent Score, Existing Controls, Residual Score, Trend (H/M/L), Owner, Target Date, Board Escalation (Y/N).\n"
                "(3) A duplicate-detection column: flag risks that appear in more than one subsidiary (e.g. cybersecurity, workforce shortage) and consolidate them as a single Group-level risk with a merged residual score.\n"
                "(4) The Top-25 Group risks ranked by residual score, with an override column for Chair-level judgment.\n\n"
                "Cite the workbook + sheet + row for every consolidated line. Do not invent risks that are not in a source register."
            )
        },
        {
            'label': '2. Analyst — tri-perspective view for the Audit Committee',
            'text': (
                "Analyst mode over the consolidated Group risk register. Produce three perspectives on the same Top-25:\n"
                "(a) Group CRO — residual-score movements vs the last cycle, top 5 velocity-of-change risks, and the 3 emerging risks not yet in any subsidiary register.\n"
                "(b) Chair of the Audit Committee — the 5 risks that need Board escalation this cycle with a specific decision ask, and the 3 risks where the Committee should challenge management on evidence quality.\n"
                "(c) Group CFO — the 5 risks with quantifiable financial exposure and the current provisioning / insurance / hedge coverage against each.\n\n"
                "Present as three side-by-side columns with a shared 'Committee action requested' column."
            )
        },
        {
            'label': '3. Cowork — Audit Committee close-out pack',
            'text': (
                "Cowork: fan out the Audit Committee close-out pack in one run. Numbers must match across every artefact:\n"
                "(1) Word — 6-page Group audit report grounded on the consolidated register + /Incident_Log_FY26.xlsx: executive summary, scope, methodology, Top-25 findings, provisioning coverage, recommendations, appendices.\n"
                "(2) PowerPoint — 10-slide Audit Committee deck (BCG/McKinsey style): cover, 5x5 heatmap, Top-25 shortlist, tri-perspective view, 5 escalations, 5 decisions requested, next-cycle plan.\n"
                "(3) Excel — the consolidated Group risk register + PivotTable by subsidiary x category x residual score.\n"
                "(4) HTML — self-contained one-page Audit Committee dashboard.\n"
                "(5) Outlook — 15-line email to Chair of Audit Committee, Group CRO, Group CFO, Group CEO, and the 4 Subsidiary CROs, with the deck attached.\n"
                "(6) Loop — live plan page for the 5 escalated risks with owner + target date + next-checkpoint.\n\n"
                "Named recipients: Chair of Audit Committee (Prof. Dato Rashid), Group CRO (Ms Aini Halim), Group CFO (Mr Wai Kong Tan), Group CEO (Dr Vera Widjaja), Subsidiary CROs (Hospitals: Dr Faisal Amran, Clinics: Ms Ratna Dewi, Diagnostics: Mr Hari Prasetyo, Insurance: Ms Nadia Sudirman)."
            )
        },
    ],
    'expected': [
...
CARDS['uc-audit-horizon-scan-fy27'] = {
    ...
    'sample_files': [
        ('Audit_Universe_FY26.xlsx', 'xlsx'),
        ('Regulator_Circulars_FY26.pdf', 'pdf'),
    ],
    'prompts': [
        {
            'label': '1. Researcher — FY27 audit-horizon scan',
            'text': (
                "Researcher Critique + Model Council. Produce a defensible horizon scan for the FY27 audit universe of a healthcare + insurance + diagnostics conglomerate. Cover: (a) regulator circulars in the last 12 months from KKM Malaysia, Kemenkes Indonesia, BPOM, KKM Singapore MOH, OJK, and IFRS 17 revisions; (b) peer internal-audit-report disclosures from listed hospital groups and life insurers in ASEAN; (c) emerging risks: AI/GenAI governance in clinical settings, cyber ransomware in hospital ITSM, drug-supply-chain-integrity post COVID-endemic, patient-data-privacy PDPA 2024, workforce shortages, ESG assurance.\n\n"
                "For each of the 10 highest-signal signals: source with citation + retrieval timestamp, plausible impact on the audit universe, and a specific audit topic that could be added to FY27. Flag any signal older than 6 months as stale."
            )
        },
        {
            'label': '2. Cowork — FY27 audit-universe pack',
            'text': (
                "Cowork: build the FY27 audit universe pack in one run. Numbers must match across every artefact:\n"
                "(1) Excel — ranked audit universe. Columns: Audit Topic, Business Line, Materiality (Rp M), Inherent Risk Score, Last-Audit-Recency (months), FY27 Score (weighted formula), Proposed FY27 Coverage (Y/N), Owner. Grounded on /Audit_Universe_FY26.xlsx + the Researcher scan.\n"
                "(2) PowerPoint — 8-slide Audit Committee planning deck: cover, horizon-scan headline, universe heatmap, ranked Top-20 with FY27 coverage flag, resource + budget ask, phasing, decisions requested, next steps.\n"
                "(3) HTML — CAE daily-briefing kanban with columns 'Watch', 'In Scope', 'In Fieldwork', 'In Reporting', 'Closed'. Seed with the Top-20 already placed on the correct column based on their status in /Audit_Universe_FY26.xlsx.\n"
                "(4) Word — 4-page audit-plan narrative for the Chair of the Audit Committee.\n"
                "(5) Outlook — email to the Chair with the deck attached.\n\n"
                "Every audit topic cites the source (workbook row, regulator circular, or Researcher scan). No fabricated topics."
            )
        },
        {
            'label': '3. Loop — live audit-plan page for FY27',
            'text': (
                "Seed a Loop page titled 'FY27 Internal Audit Plan — Live'. Sections: (1) 5x5 audit-universe heatmap embedded as a component; (2) Top-20 ranked list as a Loop table (Topic, Owner, Start, End, Status, Findings); (3) Emerging-risk watchlist (5 signals from the Researcher scan); (4) Regulator-circulars section with the 5 most material FY26 circulars; (5) Escalations section for anything that becomes Board-level during the year; (6) Owner-tagged action items for the 3 immediate next steps (kick-off meeting, resource confirmation, engagement letter template).\n\n"
                "Owners: CAE, 4 Audit Directors (Hospitals, Clinics, Diagnostics, Insurance), Head of Data Analytics for Audit."
            )
        },
    ],
    'expected': [
